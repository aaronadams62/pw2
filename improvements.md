...
| ~~5~~ | ~~**Uptime Monitoring**~~ | ✅ **Fixed #54** — Added GitHub Actions scheduled uptime checks (`.github/workflows/uptime-check.yml`) running every 12 hours for `/` and `/admin`. Uses repo variable `UPTIME_CHECK_BASE_URL` and skips cleanly until configured. |
